...
class SequenceIds implements StableIdGenerator {
  ...
  @override String next(String kind) => values.removeAt(0);
}
...
class MemoryRepository implements StudentRepository {
  ...
  StudentData value;
  bool fail;
  @override Future<StudentData> load() async => value;
  @override Future<void> save(StudentData data) async {
...
void main() {
  ...
  test('class create and rename preserve a unique stable ID', () async {
    ...
    final admin = StudentAdministration(repository: repository, ids: SequenceIds(['class-lighthouse', 'class-new']));
    ...
    expect(data.classes.last, predicate((item) => item.id == id && item.name == '10-Б'));
  });

  test('student create, edit and transfer preserve ID and contacts', () async {
    final repository = MemoryRepository(DemoRepository.data);
    final admin = StudentAdministration(repository: repository, ids: SequenceIds(['student-new']));
    var data = await admin.createStudent(repository.value, fullName: 'Тестова Учениця', classId: 'class-lighthouse', contacts: const [Contact(id: 'adult-1', name: 'Тестова Мама', role: 'Мама', phone: '+380000000000')]);
    final student = data.students.last;
    data = await admin.updateStudent(data, student.copyWith(fullName: 'Нове Ім’я', classId: 'class-horizon'));
  ...
  test('student archive records date and restore clears it', () async {
    final repository = MemoryRepository(DemoRepository.data);
    final admin = StudentAdministration(repository: repository, ids: SequenceIds([]), now: () => archivedAt);
    var data = await admin.archiveStudent(repository.value, DemoRepository.students.first.id);
  ...
  test('class with active students cannot be archived', () async {
    final repository = MemoryRepository(DemoRepository.data);
    final admin = StudentAdministration(repository: repository, ids: SequenceIds([]));
    await expectLater(admin.archiveClass(repository.value, 'class-lighthouse'), throwsA(isA<StudentValidationException>()));
  });

  test('required fields and active class are validated', () async {
    final repository = MemoryRepository(DemoRepository.data);
    final admin = StudentAdministration(repository: repository, ids: SequenceIds(['student-new']));
    await expectLater(admin.createStudent(repository.value, fullName: ' ', classId: 'class-lighthouse'), throwsA(isA<StudentValidationException>()));
    await expectLater(admin.createStudent(repository.value, fullName: 'Ім’я', classId: 'missing'), throwsA(isA<StudentValidationException>()));
  });
  ...
  test('write failure does not replace confirmed state', () async {
    ...
    final admin = StudentAdministration(repository: repository, ids: SequenceIds(['class-new']));
