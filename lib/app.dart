...
class EducatorCabinetApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) => MaterialApp(
        ...
        theme: ThemeData(colorSchemeSeed: const Color(0xff356859), brightness: Brightness.light, useMaterial3: true),
        darkTheme: ThemeData(colorSchemeSeed: const Color(0xff79b8a4), brightness: Brightness.dark, useMaterial3: true),
...
class _StudentsScreenState extends State<StudentsScreen> {
  ...
  void openStudent(Student student, bool wide) {
    if (wide) {
      ...
      Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => StudentDetail(student: student, className: repository.className(student.classId), contactAction: widget.contactAction)));
    }
  }

  @override
  Widget build(BuildContext context) => Scaffold(
        appBar: AppBar(title: const Text('Учні'), actions: const [Padding(padding: EdgeInsets.symmetric(horizontal: 16), child: Center(child: Text('ДЕМО', style: TextStyle(fontWeight: FontWeight.bold))))]),
        body: LayoutBuilder(builder: (context, constraints) {
          ...
          return Row(children: [Expanded(child: list), const VerticalDivider(width: 1), Expanded(child: Center(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 560), child: selected == null ? const Text('Оберіть учня, щоб відкрити картку') : StudentDetail(student: selected!, className: repository.className(selected!.classId), contactAction: widget.contactAction, embedded: true))))]);
        }),
      );
  ...
  Future<void> _call(String? phone, bool enabled) async {
    ...
    if (!mounted || result != ContactActionResult.copied) return;
